...
class BinarySearchTree {
  ...
  // compare the value to be inserted with the values already in the tree to
  // determine where to place it.  If the valuse if less thna the current node's value, it goes to the left; if greater, it goes to the right.
  // insert(val) {
  ...
  //     }
  //     // If the value is greater than or equal , fo right
  //     else {
  //       if (!current.right) {
  //         current = newNode;
  //         return this;
  ...
  insertRecursively(val, current = this.root) {
    ...
    //If the value to be inserted is less than the current node's value , got left
    if (val < current.val) {
      //If the left child is null, create a new node with the given value and set it as the left child
      if (current.left === null) {
        current.left = new Node(val);
        return this;
      }
      // If the left child is not null, recursively call insertRecurively on the left subtree
      return this.insertRecursively(val, current.left);
    } else {
      // If the right child is null, create a new node with the givin value and set it as the right child
      if (current.right === null) {
        current.right = new Node(val);
        return this;
      }
